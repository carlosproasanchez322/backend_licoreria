/**
 * Seed masivo — minimarket licorería
 *
 * Uso:
 *   npm run db:seed          → inserta datos (sin borrar)
 *   npm run db:seed:reset    → borra todo y vuelve a insertar
 */
...
function gradoAlcoholDe(p: (typeof PRODUCTOS_BASE)[number]): number | null {
...
async function seedCatalogos() {
  ...
  for (const nombre of METODOS_PAGO) {
    ...
    if (!existe) await prisma.metodoPago.create({ data: { nombre } });
  }
  ...
  for (const u of UNIDADES) {
    ...
    if (!existe) await prisma.unidadMedida.create({ data: u });
  }
...
async function seedUsuarios(roles: { idRol: number; nombre: string }[]) {
...
async function seedClientes(cantidad: number) {
...
async function seedProductos(
  ...
  const productosData: Parameters<typeof prisma.producto.create>[0]['data'][] = [];
  ...
  for (let i = 0; i < extras; i++) {
    const base = PRODUCTOS_BASE[i % PRODUCTOS_BASE.length];
...
async function seedCompras(
  ...
  productos: { idProducto: number; precioCompra: unknown }[],
  cantidad: number,
) {
...
async function seedVentas(
  usuarios: { idUsuario: number }[],
  clientes: { idCliente: number }[],
  metodos: { idMetodo: number }[],
  productos: { idProducto: number; precioVenta: unknown; tieneIgv: boolean; stock: number }[],
  cantidad: number,
) {
